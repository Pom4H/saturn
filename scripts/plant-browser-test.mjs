...
try {
    ...
    await check('offline PWA reload resumes persisted state, report artifacts and local revisions', async () => { await page.locator('#pause').click(); await page.waitForTimeout(500); const clock = await page.locator('#clock').innerText(); await context.setOffline(true); await page.reload(); await page.locator('#application').waitFor({ state: 'visible', timeout: 20000 }); assert.equal(await page.locator('#clock').innerText(), clock); assert.equal(await page.locator('#pause').innerText(), 'Продолжить'); assert.equal(await page.locator('[data-group]').count(), 18); assert.equal(await page.locator('[data-node]').count(), 47); await page.locator('[data-tab="reports"]').click(); await page.locator('.studio-history').evaluate(e=>e.open=true); await page.locator('#report-runs .success').first().waitFor(); await page.locator('[data-artifact]').first().click(); await page.frameLocator('#report-preview').locator('h1').waitFor(); });
    ...
    await check('Saturn inspector executes the same compiled WASM program, downloads exact CRC-tested bytes and renders its HMI',async()=>{
        ...
        assert.match(await bench.locator('#plc-front .runtime-hmi').textContent(),/300/);
