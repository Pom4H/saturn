...
Use Node.js 24 LTS (see `.nvmrc`), the Bun version pinned in `.bun-version`, and `npm ci`. Run `npm run check` and the Chromium/WebKit Playwright suite before sending a PR. The README documents browser installation and the local server.
