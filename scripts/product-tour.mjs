...
await build({ entryPoints:['plant/server.ts'], outfile:'.plant/tour-server.mjs', bundle:true, platform:'node', format:'esm', packages:'external' });
...
try {
  ...
  page.once('dialog', d => d.accept('tour.ts'));
  await page.locator('#new-file').click();
  await page.locator('#file').selectOption('tour.ts');
  await page.locator('.cm-content').click();
  await page.keyboard.insertText("// Created in Saturn engineering UI.\n// Runtime state stays outside source control.\n");
  await page.locator('#validate').click();
  await page.locator('#commit-message').fill('Create project from Saturn engineering UI');
  await page.locator('#commit').click();
  await page.waitForFunction(() => document.querySelector('#draft-state')?.textContent?.startsWith('Сохранено'));
  ...
  if (await page.locator('[data-tab=project]').isVisible()) throw new Error('operator can see project editor');
  await page.locator('[data-tab=controls]').click();
  const control = page.locator('[data-operate]').first();
  const id = await control.getAttribute('data-operate');
  const input = page.locator('[data-control-input="'+id+'"]');
  const min = Number(await input.getAttribute('min')), max = Number(await input.getAttribute('max'));
  await input.fill(String(min + (max-min)*0.55));
  await control.click();
  await pause(1800);
  ...
  if (await page.locator('[data-tab=project]').isVisible()) throw new Error('viewer can see project editor');
  await page.locator('[data-tab=scheme]').click();
  await page.locator('#fit').click();
  await pause(1600);
  await page.locator('[data-tab=controls]').click();
  if (await page.locator('[data-operate]:not([disabled])').count()) throw new Error('viewer has enabled controls');
  await pause(1500);
  ...
  console.log(JSON.stringify({ok:true, video:join(out,'saturn-engineer-operator-viewer.webm')}));
} finally {
