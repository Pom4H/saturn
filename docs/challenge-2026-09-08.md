...
| --- | --- | --- | --- |
| P1 | Удалить манометр, созданный внутри `tap(pipe(...), pressure(...))` | Вместе с прибором исчезает трубопровод, хотя он не выбран для удаления | C01, B01 |
...
```ts
import { pump, valve, pipe, tap, pressure } from "@saturn/core";
