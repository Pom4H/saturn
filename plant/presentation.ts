...
export type ViewNode =
    ...
    | { kind: 'chart'; title: string; x: string; y: string; type?: 'line'|'bar'; unit?: string }
...
export function validatePresentation(view:Presentation, target:PresentationTarget='web'):void {
    ...
    const visit=(node:ViewNode,depth:number):void=>{
        ...
        switch(node.kind){
            ...
            case 'chart':if(!text(node.title)||!key(node.x)||!key(node.y)||(node.type!==undefined&&!['line','bar'].includes(node.type))||(node.unit!==undefined&&!text(node.unit,30)))failCode('SATURN_PRESENTATION_INVALID',{reason:'invalid'},{field:'chart'});break;
...
export function renderPresentation(view:Presentation,context:PresentationContext):string {
    ...
    const render=(node:ViewNode):string=>{
        switch(node.kind){
            ...
            case 'table':return `<div class="pv-scroll"><table><thead><tr>${node.columns.map(column=>{const numeric=rows.some(row=>typeof row[column.key]==='number');return `<th${numeric?' class="pv-number"':''}>${escape(column.title)}${column.unit?` <span class="pv-unit">(${escape(column.unit)})</span>`:''}</th>`;}).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${node.columns.map(column=>{const value=row[column.key],numeric=typeof value==='number';return `<td${numeric?' class="pv-number"':''}>${escape(numeric?Number(value).toFixed(3):value)}</td>`;}).join('')}</tr>`).join('')}</tbody></table></div>`;
            case 'chart':return `<figure class="pv-chart"><figcaption>${escape(node.title)}</figcaption>${chartSVG(rows,node.x,node.y,{type:node.type,unit:node.unit})}</figure>`;
