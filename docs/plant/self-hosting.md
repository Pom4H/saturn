...
Откройте **http://127.0.0.1:4176/plant/app/**. Логин — **engineer**. При первом запуске с новой базой случайный пароль выводится в терминал один раз, в строке `Initial engineer password`. Сохраните его. Повторный запуск использует прежнюю учётную запись.
...
Тогда адрес входа — `http://127.0.0.1:4180/plant/app/`.
...
Откройте `https://saturn.example.com/plant/app/` либо в лендинге выберите «Подключить установку» и укажите `https://saturn.example.com`. Войдите с учётной записью своего сервера. Первоначальная учётная запись имеет роль инженера; интерфейс управления пользователями пока не реализован.

После входа установите PWA из меню поддерживаемого браузера. На iPhone используйте Safari → «Поделиться» → «На экран Домой». Пульт остаётся подключённым к этому серверу; для живых данных нужна сеть.
...
Перед обновлением остановите Saturn и сохраните **всю** папку данных: `/var/lib/saturn` для службы из примера или `data-plant/` для локального запуска. Нужны и SQLite, и Git-репозиторий. Копирование после остановки исключает неполный снимок работающей SQLite-базы.

Затем обновите исходники до выбранного релиза, выполните `npm ci` и `npm run plant:build` от пользователя `saturn`, запустите службу и снова проверьте `/plant/api/health`. Учитывайте совместимость версии модели с сохранённым состоянием. JSON-экспорт проекта переносит исходники, но не заменяет резервную копию базы и истории.
