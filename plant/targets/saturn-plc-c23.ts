...
export function saturnPlcC23Source(parent:BuildArtifact,controllerId:string):SaturnC23PresentationSource {
    ...
    return compileSaturnC23Presentation(controller.hmi.view);
}

export async function compileSaturnPlcC23Target(
    parent:BuildArtifact,
    controllerId:string,
    compiler:SaturnC23Compiler,
):Promise<SaturnPlcC23TargetArtifact>{
    ...
    const payload={
        schema:'saturn.target.saturn-plc-c23@1' as const,
        target:'saturn-plc-320' as const,
        parentBuildHash:parent.hash,
        sourceRevision:parent.provenance.sourceRevision??null,
        controllerId,
        presentationId:source.viewId,
