...
export function validatePresentation(view:Presentation, target:'web'|'report'|'plc'='web'):void {
    ...
    const visit=(node:ViewNode,depth:number):void=>{
        ...
        switch(node.kind){
            ...
            case 'chart':if(!text(node.title)||!key(node.x)||!key(node.y)||(node.type!==undefined&&!['line','bar'].includes(node.type))||(node.unit!==undefined&&!text(node.unit,30)))throw new AppError('Invalid presentation chart');break;
