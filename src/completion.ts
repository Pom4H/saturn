...
import { componentRegistry, type Scene } from './core';
...
export function dslCompletions(source: string, position: number, scene?: Scene): Completion[] {
  ...
  if (member) {
    ...
    if (node) return Object.entries(componentRegistry.schematic(node.kind).ports).map(([label, port]) => ({ label, type: 'property', detail: `${port.role} · ${port.direction}` }));
  }
  ...
  if (call && ts.isIdentifier(call.expression)) {
    ...
    const definition = componentRegistry.findSchematic(kind);
    if (definition) {
  ...
  return [
    ...[...componentRegistry.kinds(), 'component', 'runtime', 'connect', 'tap'].map(label => ({ label, type: 'function', detail: '@scada/core' })),
