...
const helpArticles: readonly HelpArticle[] = [
  ...
  { id: 'server', title: 'Проект с сервера', keywords: 'git ревизия авторизация обновление черновик публикация', paragraphs: [
    '«С сервера» загружает файлы из одной Git-ревизии через текущую серверную сессию. Войти можно через ссылку в окне подключения; для API проекта требуется роль engineer.',
    'Обновление не перезаписывает ваш изменённый черновик. Сначала сохраните его отдельной локальной копией или отмените изменения, затем примените новую ревизию.',
    'Серверные документы остаются в памяти вкладки. Они не сохраняются автоматически в локальные проекты или публичный офлайн-кэш. Перед закрытием скачайте изменения или создайте локальную копию.',
    'Этот Shell пока редактирует проект: серверное исполнение, commit и publish доступны в отдельном серверном приложении.',
  ] },
  ...
  { id: 'limits', title: 'Что подключено сейчас', keywords: 'ограничения демо runtime телеметрия контроллер', paragraphs: [
    ...
    'Shell поддерживает исходники, многофайловую компиляцию, 2D/3D и загрузку проекта с сервера. Серверная симуляция передаёт значения и тревоги в 2D/3D и таблицу сигналов через SSE. Публикация правок из Shell пока не подключена.',
...
function renderHelp(): void {
  ...
  if (article.id === 'server') {
    const link = el('a', 'shell-help-link', 'Открыть серверное приложение'); link.href = '/plant/app/'; link.target = '_blank'; link.rel = 'noopener'; helpContent.append(link);
  }
}
