...
try {
    ...
    await check('a second tab cannot silently become another database writer', async () => { const tab = await context.newPage(); await tab.goto(base + 'demo/'); await tab.locator('#memory').waitFor({ state: 'visible', timeout: 15000 }); const message = await tab.locator('#loading').innerText(); assert.match(message, /другой вкладке/); assert.doesNotMatch(message, /browser\.database|state changed since the request was prepared/); await tab.close(); });
