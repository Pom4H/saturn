...
export function chartSVG(rows: Record<string, unknown>[], x: string, y: string, options: ChartOptions = {}): string {
    const source = rows.map((row, index) => {
        ...
        return {
            ...
            rawX,
            numericX,
            y: numericY !== null && Number.isFinite(numericY) ? numericY : null,
        };
    });
    const good = source.filter(point => point.y !== null);
    if (!good.length)
        return '<p class="pv-empty">Нет достоверных данных для графика.</p>';
    ...
    const yMin = options.type === 'bar' ? Math.min(0, dataMin) : Math.min(0, dataMin);
    ...
    const xTicks = source.filter((_, index) => index % tickStep === 0 || index === source.length - 1).map(point => {
        const xx = px(point);
        const label = typeof point.rawX === 'number' && Number.isInteger(point.rawX)
            ? String(point.rawX).padStart(2, '0')
            : String(point.rawX);
    ...
    if ((options.type ?? 'line') === 'bar') {
        ...
        const barWidth = Math.max(3, Math.min(28, plotWidth / slots * .64));
        marks = source.map(point => {
            if (point.y === null) return '';
            const xx = px(point) - barWidth / 2;
            const yy = py(point.y);
            const rectY = Math.min(yy, baseline);
            const rectHeight = Math.max(1, Math.abs(baseline - yy));
