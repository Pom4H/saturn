...
test('schema migration runs before domain services and preserves an existing database', () => {
    const db = new NodeSql();
    const store = new Store(db);
    assert.equal(db.all<{ user_version: number }>('PRAGMA user_version')[0].user_version, 0);
    db.exec('CREATE TABLE meta(key TEXT PRIMARY KEY,value TEXT NOT NULL)');
    db.exec("INSERT INTO meta VALUES('existing','42')");
    migrate(db);
    assert.equal(store.meta('existing', 0), 42);
    assert.equal(db.all<{ user_version: number }>('PRAGMA user_version')[0].user_version, 1);
    migrate(db);
    assert.equal(store.meta('existing', 0), 42);
    db.close();
});
const makeService = async (files = demoFiles) => { const store = openStore(); let serial = 0; const service = new Service(store, { now: () => 1000000, uuid: () => `id-${++serial}`, reportRunner: async (task) => executeReport(task, new NodeSql()) }); await service.start(await buildArtifact(files, { packageName: '@saturn/test' })); return service; };
...
test('physical topology diagnostics keep stable codes and localize presentation', () => {
    ...
    assert.throws(() => validateProject(p), error => {
        ...
        const payload = errorPayload(error, 'ru');
        assert.equal(payload.code, 'SATURN_CONNECTION_INCOMPATIBLE');
        assert.equal(payload.messageKey, 'ports.incompatibleConnection');
        assert.equal(payload.locale, 'ru');
        assert.equal(payload.data?.connectionKind, 'pipe');
        assert.match(payload.error, /Несовместимое соединение/);
        return true;
    });
});
...
test('archive keeps step segments, quality and predecessor at range boundary', () => { const store = openStore(); for (const [time, value, quality] of [[0, 10, 'good'], [1000, 20, 'good'], [3000, null, 'offline'], [4000, 20, 'good']] as const)
...
test('Web Push allowlist, per-session subscription, expired endpoint handling', async () => { assert.equal(allowedPushEndpoint('https://127.0.0.1/api'), false); assert.equal(allowedPushEndpoint('http://fcm.googleapis.com/x'), false); assert.equal(allowedPushEndpoint('https://fcm.googleapis.com.evil.test/x'), false); const store = openStore(), auth = new Auth(store); auth.seed('engineer', 'password-for-tests'); const login = auth.login('engineer', 'password-for-tests', 'local'), session = auth.session(`scada_session=${login.token}`); const ecdh = createECDH('prime256v1'); ecdh.generateKeys(); const subscription = { endpoint: 'https://fcm.googleapis.com/fcm/send/test', keys: { p256dh: ecdh.getPublicKey().toString('base64url'), auth: randomBytes(16).toString('base64url') } }; let sent = 0; const sendExpired: PushSender = async (_subscription, payload) => { sent++; assert.ok(!String(payload).includes('temperature')); throw Object.assign(new Error('Expired'), { statusCode: 410 }); };
...
test('HTTP auth, CSRF, private HTML, SQL reports, SSE and revocation', async () => { const dir = await mkdtemp(join(tmpdir(), 'scada-http-test-')); const app = await startPlantServer({ port: 0, data: join(dir, 'db.sqlite'), password: 'password-for-http-tests', autoTick: false }); try {
    ...
    assert.ok((await page.text()).includes('Saturn'));
...
test('push survives cookie expiration, but explicit logout revokes the device subscription', async () => { const store = openStore(), auth = new Auth(store); auth.seed('engineer', 'password-for-expiry-tests'); const login = auth.login('engineer', 'password-for-expiry-tests', 'local'), session = auth.session(`scada_session=${login.token}`); const key = createECDH('prime256v1'); key.generateKeys(); let sent = 0; const sendAccepted: PushSender = async () => { sent++; return { statusCode: 201 } as Awaited<ReturnType<PushSender>>; };
