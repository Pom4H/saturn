...
| `SCADA_UNIX_SOCKET` | Listen on a Unix-domain socket instead of HOST/PORT; requires `SCADA_PUBLIC_URL` |
| `SCADA_TLS_CERT`, `SCADA_TLS_KEY` | Serve HTTPS directly from Bun; configure both together |
| `SCADA_TLS_CA` | Optional comma-separated CA certificate paths for direct TLS |
| `SCADA_HTTP2` | Override direct server HTTP/2; enabled by default when TLS is configured |
| `SCADA_REPORT_CGROUP` | Optional Linux cgroup path for disposable report workers |

Do not expose plaintext HTTP remotely. Either terminate TLS before the server or configure Bun's direct TLS listener, and set `SCADA_PUBLIC_URL` to the browser-visible HTTPS origin. Authentication is server-side: new Bun-server credentials use Argon2id, existing scrypt credentials upgrade after successful login, and HttpOnly/SameSite session cookies, CSRF tokens, same-origin checks, named roles, login throttling, and server-side authorization protect mutations. `/plant/app/`, project sources, history, reports and SSE require authentication. Only the public demonstration and build assets are anonymous. This is single-installation authorization, not multi-tenant ACLs. User administration UI and external identity federation are not included.
