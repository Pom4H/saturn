...
const supply = installed({ kind: 'supply', titleKey: 'model.supply', visual: 'generator', inputs: {}, parameters: { voltage: p(1, 0, 1.5) }, outputs: { voltage: 'отн.' }, outputDimensions: { voltage: 'voltage' } as const,
    initialize: q => ({ voltage: q.voltage }), advance: (_s, _i, q) => ({ voltage: q.voltage }), observe: s => ({ ...s }) });
const pump = installed({ kind: 'pump', titleKey: 'model.pump', visual: 'pump', inputs: { voltage: 1, resistance: 1 }, inputDimensions: { voltage: 'voltage', resistance: 'resistance' } as const, parameters: { inertia: p(6, .1, 120), nominalFlow: p(1, .01, 100) }, outputs: { flow: 'отн.', rpm: 'об/мин', power: 'отн.' }, outputDimensions: { flow: 'flow', rpm: 'rotational-speed', power: 'power' } as const,
...
const flowmeter = installed({ kind: 'flowmeter', titleKey: 'model.flowmeter', visual: 'flowmeter',
    inputs: { flow: 0 }, inputDimensions: { flow: 'flow' } as const,
    parameters: { lag: p(.2, .01, 10) }, outputs: { flow: 'отн.' }, outputDimensions: { flow: 'flow' } as const,
    initialize: () => ({ flow: 0 }),
    advance: (state, input, q, dt) => ({ flow: approach(state.flow, input.flow, dt, q.lag) }),
    observe: state => ({ flow: state.flow }) });
/** Dimensionless feedback heat source, NOT neutron kinetics or a reactor operating model. */
const feedback = installed({ kind: 'feedback-source', titleKey: 'model.feedback-source', visual: 'reactor', inputs: { void: 0, temperature: 1, absorber: 0 }, parameters: { feedback: p(3, 0, 8), thermalFeedback: p(.35, 0, 4), response: p(4, .2, 60), initialPower: p(1, .001, 5) }, outputs: { power: 'отн.', rate: 'отн./с' },
...
const separator = installed({ kind: 'separator', titleKey: 'model.separator', visual: 'separator', inputs: { heat: 1, demand: 1 }, parameters: { capacity: p(8, .1, 100) }, outputs: { pressure: 'отн.', steam: 'отн.', level: '%' },
...
const turbine = installed({ kind: 'turbine', titleKey: 'model.turbine', visual: 'turbine', inputs: { steam: 1, load: 1 }, parameters: { inertia: p(12, .5, 120), efficiency: p(.9, 0, 1) }, outputs: { rpm: 'об/мин', electricity: 'отн.', exhaust: 'отн.' },
...
const exchanger = installed({ kind: 'heat-exchanger', titleKey: 'model.heat-exchanger', visual: 'exchanger', inputs: { heat: .1, cooling: 1 }, inputDimensions: { heat: 'power', cooling: 'flow' } as const, parameters: { capacity: p(12, .1, 100), ambient: p(.2, 0, 1) }, outputs: { temperature: 'отн.', rejected: 'отн.' }, outputDimensions: { temperature: 'temperature', rejected: 'power' } as const,
...
const sensor = installed({ kind: 'sensor', titleKey: 'model.sensor', visual: 'sensor', inputs: { value: 0 }, parameters: { lag: p(.2, .02, 30), bias: p(0, -100, 100) }, outputs: { value: 'отн.' }, initialize: () => ({ value: 0 }), advance: (s, i, q, dt) => ({ value: approach(s.value, i.value + q.bias, dt, q.lag) }), observe: s => ({ ...s }) });
const protection = installed({ kind: 'protection', titleKey: 'model.protection', visual: 'control', inputs: { temperature: 1, power: 1, demand: 0 }, parameters: { temperatureLimit: p(1.7, 1, 5), powerLimit: p(2, 1, 10), actuation: p(1, .05, 60) }, outputs: { insertion: 'доля', trip: 'лог.' }, outputTypes: { trip: 'boolean' } as const,
...
const structure = installed({ kind: 'structure', titleKey: 'model.structure', visual: 'structure', inputs: { release: 0 }, parameters: { capacity: p(12, .1, 100), vent: p(.3, 0, 10), strength: p(1.4, .1, 10) }, outputs: { pressure: 'отн.', damage: 'доля' },
...
const reservoir = installed({ kind: 'reservoir', titleKey: 'model.reservoir', visual: 'reservoir', inputs: { inflow: .1, demand: .1 }, inputDimensions: { inflow: 'flow', demand: 'flow' } as const,
...
const valve = installed({ kind: 'motor-valve', titleKey: 'model.motor-valve', visual: 'valve', inputs: { demand: .5, pressure: 1 }, inputDimensions: { demand: 'ratio', pressure: 'ratio' } as const,
...
const ups = installed({ kind: 'ups', titleKey: 'model.ups', visual: 'battery', inputs: { grid: 1, demand: .1 },
...
const switchgear = installed({ kind: 'switchgear', titleKey: 'model.switchgear', visual: 'switchgear', inputs: { voltage: 1, demand: 1, load: .1 },
...
const fan = installed({ kind: 'fan', titleKey: 'model.fan', visual: 'fan', inputs: { voltage: 1, demand: .8 }, inputDimensions: { voltage: 'voltage', demand: 'ratio' } as const,
...
const motor = installed({ kind: 'electric-motor', titleKey: 'model.electric-motor', visual: 'motor',
...
const tower = installed({ kind: 'cooling-tower', titleKey: 'model.cooling-tower', visual: 'tower',
...
const filter = installed({ kind: 'strainer', titleKey: 'model.strainer', visual: 'filter',
...
const checkValve = installed({ kind: 'check-valve', titleKey: 'model.check-valve', visual: 'checkvalve',
...
const vessel = installed({ kind: 'expansion-vessel', titleKey: 'model.expansion-vessel', visual: 'accumulator',
...
const relief = installed({ kind: 'relief-valve', titleKey: 'model.relief-valve', visual: 'relief',
...
const transformer = installed({ kind: 'transformer', titleKey: 'model.transformer', visual: 'transformer',
...
const alternator = installed({ kind: 'alternator', titleKey: 'model.alternator', visual: 'alternator',
...
const thermalStore = installed({ kind: 'thermal-store', titleKey: 'model.thermal-store', visual: 'calorimeter',
...
const dcSupply=installed({kind:'dc-supply',visual:'dcSupply',titleKey:'model.dc-supply',inputs:{enable:1},inputDimensions:{enable:'ratio'} as const,parameters:{voltage:p(24,12,24)},outputs:{voltage:'V',return:'V'},outputDimensions:{voltage:'voltage',return:'voltage'} as const,initialize:()=>({voltage:0,return:0}),advance:(_s,i,q)=>({voltage:i.enable>0?q.voltage:0,return:0}),observe:s=>({...s})});
const transmitter=installed({kind:'transmitter',visual:'transmitter',titleKey:'model.transmitter',inputs:{value:0},parameters:{gain:p(1,0,1000)},outputs:{value:'отн.'},initialize:()=>({value:0}),advance:(_s,i,q)=>({value:i.value*q.gain}),observe:s=>({...s})});
const contactor=installed({kind:'contactor',visual:'contactor',titleKey:'model.contactor',inputs:{coil:0,supply:0},parameters:{travel:p(.1,.02,1)},outputs:{voltage:'V',closed:'доля'},initialize:()=>({voltage:0,closed:0}),advance:(s,i,q,dt)=>{const closed=approach(s.closed,i.coil>0?1:0,dt,q.travel);return{closed,voltage:closed>.8?i.supply:0};},observe:s=>({...s})});
const indicator=installed({kind:'indicator',visual:'indicator',titleKey:'model.indicator',inputs:{voltage:0},parameters:{nominal:p(24,12,24)},outputs:{brightness:'доля'},initialize:()=>({brightness:0}),advance:(_s,i,q)=>({brightness:clamp(i.voltage/q.nominal)}),observe:s=>({...s})});
const ioModule=installed({kind:'io-module',visual:'ioModule',titleKey:'model.io-module',inputs:{channel1:0,channel2:0,channel3:0,channel4:0},parameters:{},outputs:{channel1:'отн.',channel2:'отн.',channel3:'отн.',channel4:'отн.'},initialize:()=>({channel1:0,channel2:0,channel3:0,channel4:0}),advance:(_s,i)=>({...i}),observe:s=>({...s})});
const junction=installed({kind:'junction',visual:'junction',titleKey:'model.junction',inputs:{flow:0},parameters:{},outputs:{flow:'отн.'},initialize:()=>({flow:0}),advance:(_s,i)=>({flow:i.flow}),observe:s=>({...s})});
export const builtInModels = { 'dc-supply':dcSupply,transmitter,contactor,indicator,'io-module':ioModule,junction, supply, pump, flowmeter, 'feedback-source': feedback, channel, separator, turbine, 'heat-exchanger': exchanger, sensor, protection, structure, reservoir, 'motor-valve': valve, ups, switchgear, fan, 'electric-motor': motor, 'cooling-tower': tower, strainer: filter, 'check-valve': checkValve, 'expansion-vessel': vessel, 'relief-valve': relief, transformer, alternator, 'thermal-store': thermalStore };
