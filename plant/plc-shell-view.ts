...
export function drawPlcShell(svg:SVGSVGElement,project:Project,frame:Frame,controllerId:string,index=0){
 ...
 const lamp=svg.querySelector<SVGCircleElement>('[data-lamp]');if(lamp){const v=project.devices.find(d=>d.type==='indicator'&&d.system===project.controllers!.find(c=>c.id===controllerId)!.system);const on=(v?sample(frame,v.id+'.brightness'):sample(frame,controllerId+'.DO1')??0)>.5;lamp.setAttribute('class','ps-lamp '+(on?'on':''));}
