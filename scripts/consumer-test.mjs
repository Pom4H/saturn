...
try {
  await symlink(resolve('packages/core'), `${modules}/@saturn/core`, 'junction');
  ...
  if (result.status !== 0) process.exitCode = result.status ?? 1;
    ...
    await build({
      entryPoints: ['examples/consumer/index.ts'],
      outfile: '.test/consumer.mjs',
      bundle: true,
      platform: 'node',
      format: 'esm',
      target: 'node24',
    ...
    result = spawnSync(process.execPath, ['.test/consumer.mjs'], { stdio: 'inherit' });
