...
const hmiEquipment: Record<string, HmiDomEquipmentRenderer> = {
    'saturn-plc': (node, runtime) => {
        ...
        card.dataset.controllerId = node.equipmentId;
...
document.addEventListener('click',e=>{
 const key=(e.target as Element).closest<HTMLElement>('[data-plc-button]');if(!key)return;
 const controller=key.closest<HTMLElement>('[data-controller-id]')?.dataset.controllerId;
 if(!controller)return;e.preventDefault();e.stopPropagation();key.classList.add('is-pressed');setTimeout(()=>key.classList.remove('is-pressed'),140);
 void guard(async()=>{await command('plc-key',{target:controller,parameter:key.dataset.plcButton});});
});
document.addEventListener('keydown',e=>{const key=(e.target as Element).closest<HTMLElement>('[data-plc-button]');if(key&&(e.key==='Enter'||e.key===' ')){e.preventDefault();key.click();}});
