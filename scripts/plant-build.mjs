...
const raw = { name: 'project-source', setup(b) { b.onResolve({ filter: /\?raw$/ }, a => { const source=a.path.slice(0,-4); return { path: isAbsolute(source) ? source : resolve(a.resolveDir, source), namespace: 'raw' }; }); b.onLoad({ filter: /.*/, namespace: 'raw' }, async (a) => ({ contents: await readFile(a.path, 'utf8'), loader: 'text' })); } };
...
await build({ ...options, entryPoints: ['plant/adapters/wasm-sandbox-worker.ts'], outfile: '.plant/wasm-sandbox-worker.mjs', platform: 'node', packages: 'external' });
await build({ ...options, entryPoints: ['plant/worker.ts'], outfile: '.plant/worker.mjs', platform: 'node', packages: 'external' });
...
console.log('Built Saturn server/worker for Node or Bun and browser-only /plant/demo/');
